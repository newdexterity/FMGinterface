...
class ProcessData:
	...
	def __init__(self, filenames, data_split, frequency, data_end_drop, features_end=None, labels=None):
		home = os.path.expanduser("~")
		self.path = os.path.join(home, 'experiments', 'fsr_band')
		# CSV Attributes
	...
	def read_data_classification(self):
		...
		for i in self.filenames:
			...
			labels = self.data[:, 6] * (file_counter + 1)
			labels = np.reshape(labels, [len(labels), 1])
			# Last Col is Label
			emg_channels = self.data[:, :6]
...
class Classifiers:
	...
	def random_forest_train(self):
		...
		filename = os.path.join(self.path, self.name.split('.')[0] + '.sav')
...
if __name__ == '__main__':
	r = Classifiers((['pinch_AD_1_features.csv', 'tripod_AD_1_features.csv', 'power_AD_1_features.csv', 'extension_AD_1_features.csv']),
					split=10, frequency=1200, data_end_drop=0, num_trees=100, name='right_gesture_classifier')
